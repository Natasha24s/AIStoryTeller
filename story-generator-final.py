...
- Keep descriptions concise and image-friendly"""
        ...
        # Using Claude 3 Sonnet via Amazon Bedrock (standard version)
        ...
        # Get the generated text from Claude via Bedrock
        story_text = response["output"]["message"]["content"][0]["text"]
        
        # More sophisticated parsing to handle different scene formats
        ...
        # Remove any title lines (short lines that might be scene titles)
...
def handler(event, context):
    ...
    try:
        ...
        for idx, scene in enumerate(scenes):
            ...
            # Enhance scene description with character consistency
...
#            'images': images,
            'image_urls': image_urls,
            'metadata': metadata,
            'characters': characters  # Include character information in response
        }
...
def extract_character_details(story_text):
    ...
    # Simple character detection (you might want to use more sophisticated NLP)
    ...
    for scene in scenes:
        ...
        for match in matches:
            ...
            if name not in characters:
                # Find the sentence containing the character's first mention
